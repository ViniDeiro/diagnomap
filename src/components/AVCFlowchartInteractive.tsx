...
export type AVCCaseData = {
  ...
  vascularImagingAvailability?: 'disponivel' | 'indisponivel_transferir' | 'indisponivel_local'
  ...
  postThrombolysisBloodPressure?: string
  postThrombolysisBPManagement?: string[]
...
const postThrombolysisBPOptions = [
  ['monitoring', 'Intensificar monitorização da PA', 'Aferir a cada 15 minutos nas primeiras 2 h, a cada 30 minutos nas 6 h seguintes e, depois, a cada hora até completar 24 h.'],
  ['clevidipine', 'Clevidipina em infusão titulada', 'Referência: iniciar 1–2 mg/h e titular a cada 2–5 min, até 21 mg/h.'],
  ['local_protocol', 'Aplicar alternativa do protocolo institucional', 'Registrar fármaco, dose, resposta e motivo da escolha, com monitorização contínua.']
] as const
...
const CardOption = ({ selected, title, description, danger, disabled, onClick }: { selected: boolean; title: string; description?: string; danger?: boolean; disabled?: boolean; onClick: () => void }) => (
  <button
    type="button"
    ...
    className={clsx(
      ...
      selected
        ...
        : disabled ? 'border-slate-200 bg-slate-100' : 'border-slate-200 bg-white hover:border-indigo-300 hover:bg-slate-50'
    )}
  >
...
const AVCFlowchartInteractive: React.FC<AVCFlowchartInteractiveProps> = ({
  ...
  const parsedCurrentBloodPressure = parseAVCBloodPressure(data.currentBloodPressure)
  const pressureWithinThrombolysisLimit = isAVCBloodPressureWithinThrombolysisLimit(data.currentBloodPressure)
  ...
  const nextFromWindow = (windowValue: AVCCaseData['timeWindow']) => {
    ...
    else if (windowValue === '6_9h' || windowValue === '9_24h') persist('avc_vaso', { timeWindow: windowValue, vascularImagingAvailability: undefined, vesselTerritory: undefined })
        ...
        <main className="p-5 sm:p-8">
          ...
          {stage === 'avc_imagem_avancada' && (
            <div className="space-y-4">
              ...
              <button type="button" disabled={!data.advancedImaging} onClick={() => data.advancedImaging === 'mismatch' ? persist('avc_trombolise_seguranca') : data.advancedImaging === 'indisponivel_transferir' ? persist('avc_transferencia_reperfusao') : data.advancedImaging === 'indisponivel' ? persist('avc_cuidados_sem_reperfusao') : persist('avc_vaso', { vascularImagingAvailability: data.advancedImaging === 'indisponivel_angio_disponivel' ? 'disponivel' : undefined, vesselTerritory: undefined })} className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-700 px-5 py-4 font-extrabold text-white disabled:bg-slate-300">Aplicar disponibilidade e continuar <ChevronRight /></button>
            </div>
          )}

          {stage === 'avc_trombolise_seguranca' && (
            <div className="space-y-6">
              <section><h2 className="text-lg font-black text-red-950">Impedimentos maiores</h2><div className="mt-3 grid gap-3 md:grid-cols-2">{absoluteContraindications.map(([id,label]) => <CardOption key={id} selected={(data.thrombolysisContraindications || []).includes(id)} title={label} danger onClick={() => selectMany('thrombolysisContraindications', id)} />)}</div></section>
              <section><h2 className="text-lg font-black text-amber-950">Alertas para decisão individualizada</h2><div className="mt-3 grid gap-3 md:grid-cols-2">{relativeAlerts.map(([id,label]) => <CardOption key={id} selected={(data.thrombolysisContraindications || []).includes(id)} title={label} onClick={() => selectMany('thrombolysisContraindications', id)} />)}</div></section>
              <div className="grid gap-3 md:grid-cols-2">
                <label className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm font-black text-slate-900">
                  Pressão arterial atual
                  <input
                    value={data.currentBloodPressure || ''}
                    onChange={event => {
                      const currentBloodPressure = event.target.value
                      update({
                        currentBloodPressure,
                        ...(!isAVCBloodPressureWithinThrombolysisLimit(currentBloodPressure) ? { pressureReadyForThrombolysis: false } : {})
                      })
                    }}
                    placeholder="Ex.: 170/100"
                    className="mt-3 w-full rounded-xl border border-slate-300 bg-white px-4 py-3"
                  />
                </label>
                <CardOption
                  selected={pressureWithinThrombolysisLimit && Boolean(data.pressureReadyForThrombolysis)}
                  title="PA abaixo ou igual a 185/110 mmHg"
                  description={pressureWithinThrombolysisLimit ? 'Valor compatível. Confirme a aferição para liberar a próxima etapa.' : 'Opção bloqueada até registrar pressão sistólica ≤185 e diastólica ≤110 mmHg.'}
                  danger={!data.pressureReadyForThrombolysis}
                  disabled={!pressureWithinThrombolysisLimit}
                  onClick={() => update({ pressureReadyForThrombolysis: !data.pressureReadyForThrombolysis })}
                />
              </div>
              ...
              {parsedCurrentBloodPressure && !pressureWithinThrombolysisLimit && <div className="space-y-4 rounded-2xl border border-red-300 bg-red-50 p-4 text-sm text-red-950"><p className="font-semibold">PA acima do limite para trombólise IV. A confirmação ficará bloqueada até nova aferição documentar valor ≤185/110 mmHg.</p><div className="rounded-xl border border-red-200 bg-white p-4"><p className="font-black">Opções EV conforme disponibilidade e protocolo brasileiro</p><ul className="mt-2 list-disc space-y-1 pl-5"><li><strong>Metoprolol:</strong> 5 mg EV lentamente; reavaliar e repetir conforme protocolo e resposta, respeitando contraindicações e dose máxima institucional.</li><li><strong>Hidralazina:</strong> utilizar conforme protocolo neurovascular local, com monitorização e nova aferição.</li><li><strong>Alternativa institucional:</strong> outro agente pode ser empregado conforme protocolo e disponibilidade, com monitorização contínua. Nitroprussiato exige monitorização rigorosa e não é primeira escolha rotineira quando houver preocupação com pressão intracraniana.</li></ul></div><div className="flex justify-end"><ConductCopyButton text={`CONDUTA — PA ACIMA DO LIMITE PARA TROMBÓLISE IV\nPA registrada: ${data.currentBloodPressure}. Meta antes da trombólise: ≤185/110 mmHg.\nOpções conforme protocolo brasileiro e disponibilidade: metoprolol 5 mg EV lentamente, com reavaliação e repetição conforme protocolo; hidralazina conforme protocolo neurovascular; ou outro agente previsto no protocolo institucional.\nReaferir pressão, evitar queda abrupta e somente liberar trombólise após documentar valor dentro da meta.`} /></div></div>}
              ...
              <div className="grid gap-3 sm:grid-cols-2"><button type="button" disabled={thrombolysisBlocked} onClick={() => persist('avc_trombolitico')} className="rounded-xl bg-emerald-700 px-5 py-4 font-extrabold text-white disabled:bg-slate-300">Elegível: escolher trombolítico</button><button type="button" onClick={() => persist('avc_vaso', { vascularImagingAvailability: undefined, vesselTerritory: undefined })} className="rounded-xl border border-indigo-300 bg-indigo-50 px-5 py-4 font-extrabold text-indigo-900">Sem trombólise IV: avaliar trombectomia ou indisponibilidade</button></div>
            </div>
          )}
          ...
          {stage === 'avc_pos_trombolise' && (
            <div className="space-y-5">
              <div className="rounded-2xl border border-blue-200 bg-blue-50 p-5 text-sm text-blue-950"><h3 className="font-black">Primeiras 24 horas</h3><ul className="mt-2 list-disc space-y-1 pl-5"><li>NIHSS e PA seriados: a cada 15 min nas primeiras 2 h, a cada 30 min nas 6 h seguintes e, depois, a cada hora até 24 h.</li><li>Manter PA abaixo de 180/105 mmHg, evitando redução abrupta e meta intensiva de PAS abaixo de 140 mmHg.</li><li>Evitar punções e dispositivos invasivos não indispensáveis.</li><li>Não iniciar antiagregante ou anticoagulante antes da imagem de controle em 24 h.</li></ul></div>
          ...
          {stage === 'avc_vaso' && (
            <div className="space-y-5">
              <section className="rounded-2xl border border-blue-200 bg-blue-50 p-5 text-blue-950">
                <p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">Disponibilidade do serviço</p>
                <h2 className="mt-1 text-xl font-black">Há angio-TC ou outra angioimagem disponível agora?</h2>
                <p className="mt-2 text-sm leading-relaxed">Não presuma disponibilidade. A ausência do exame não pode travar o atendimento nem ser registrada como ausência de oclusão.</p>
              </section>
              <div className="grid gap-3 md:grid-cols-3">
                <CardOption selected={data.vascularImagingAvailability === 'disponivel'} title="Sim — angioimagem realizada ou disponível" description="Liberar a seleção do território vascular conforme o resultado do exame." onClick={() => update({ vascularImagingAvailability: 'disponivel', advancedImaging: data.advancedImaging === 'indisponivel' ? undefined : data.advancedImaging })} />
                <CardOption selected={false} title="Não disponível — transferência é possível" description="Acionar neurologia/regulação e centro vascular/endovascular sem interromper suporte e monitorização." danger onClick={() => persist('avc_transferencia_reperfusao', { vascularImagingAvailability: 'indisponivel_transferir', advancedImaging: 'indisponivel_transferir', vesselTerritory: undefined })} />
                <CardOption selected={false} title="Não disponível — transferência não é factível" description="Documentar a limitação e avançar para manejo clínico completo, com destino conforme estabilidade." danger onClick={() => persist('avc_cuidados_sem_reperfusao', { vascularImagingAvailability: 'indisponivel_local', advancedImaging: 'indisponivel', vesselTerritory: undefined })} />
              </div>
              {data.vascularImagingAvailability === 'disponivel' && (
                <section className="space-y-4 rounded-2xl border border-indigo-200 bg-indigo-50/50 p-4">
                  <div><p className="text-xs font-black uppercase tracking-[0.16em] text-indigo-700">Resultado da angioimagem</p><h2 className="mt-1 text-lg font-black text-indigo-950">Selecione o território identificado</h2></div>
                  <div className="grid gap-3 md:grid-cols-2">{([['grande_anterior','Grande vaso da circulação anterior','Carótida interna ou segmento proximal da cerebral média.'],['m2_dominante','Ramo M2 dominante','Oclusão de médio vaso com território funcional relevante.'],['medio_distal','Médio vaso distal ou não dominante','Evidência de benefício rotineiro é insuficiente no protocolo.'],['basilar','Artéria basilar','Aplicar PC-ASPECTS, NIHSS e funcionalidade prévia.'],['sem_ogv','Sem oclusão tratável de grande vaso','Seguir cuidados clínicos e prevenção secundária.']] as const).map(([id,title,description]) => <CardOption key={id} selected={data.vesselTerritory === id} title={title} description={description} danger={id === 'medio_distal'} onClick={() => id === 'sem_ogv' ? persist('avc_cuidados_sem_reperfusao', { vesselTerritory: id }) : update({ vesselTerritory: id })} />)}</div>
                  {data.vesselTerritory && data.vesselTerritory !== 'sem_ogv' && <button type="button" onClick={() => persist('avc_trombectomia_criterios')} className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-700 px-5 py-4 font-extrabold text-white">Aplicar critérios de trombectomia <ChevronRight /></button>}
                </section>
              )}
            </div>
          )}
