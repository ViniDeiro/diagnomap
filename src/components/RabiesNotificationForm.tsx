...
import { CheckCircle2, ChevronDown, ClipboardList, Download, Info, Loader2, Printer } from 'lucide-react'
...
const RabiesNotificationForm: React.FC<Props> = ({ patient, value = {}, outcome, onChange }) => {
  ...
  const handlePdf = async (action: 'save' | 'print') => {
    ...
    setPdfError('')
    try {
      if (action === 'save') await saveRabiesSinanPdf({ patient, notification: value })
      else await printRabiesSinanPdf({ patient, notification: value })
    } catch (error) {
  ...
  return <section className="overflow-hidden rounded-[1.75rem] border border-indigo-200 bg-indigo-50/40 shadow-sm">
    <div className="border-b border-indigo-200 bg-gradient-to-r from-indigo-800 to-blue-700 p-5 text-white sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="rounded-xl bg-white/15 p-2.5"><ClipboardList className="h-6 w-6" /></span><div><p className="text-xs font-black uppercase tracking-[0.18em] text-indigo-100">Ficha de investigação SINAN</p><h2 className="mt-1 text-xl font-black">Atendimento antirrábico humano</h2><p className="mt-1 text-sm text-indigo-100">Todos os campos são opcionais. Se forem preenchidos, os dados serão aplicados à ficha oficial para impressão ou PDF.</p></div></div><span className="w-fit rounded-full bg-white/15 px-3 py-2 text-xs font-black uppercase tracking-wider ring-1 ring-white/20">Preenchimento opcional</span></div></div>
    <div className="space-y-3 p-4 sm:p-5">
      <div className="flex gap-2 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-950"><Info className="mt-0.5 h-4 w-4 shrink-0" /><p>Nome, nascimento, idade e sexo vêm do cadastro do paciente. Nenhum campo desta ficha bloqueia a finalização do atendimento.</p></div>
      {sections.map(([id, label]) => <div key={id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white"><button type="button" aria-expanded={open === id} onClick={() => setOpen(open === id ? '' : id)} className="flex w-full items-center justify-between gap-3 px-4 py-4 text-left font-black text-slate-950"><span>{label}</span><ChevronDown className={clsx('h-5 w-5 text-slate-500 transition-transform', open === id && 'rotate-180')} /></button>{open === id && <div className="border-t border-slate-200 p-4 sm:p-5">{sectionBody(id)}</div>}</div>)}
      <div className="rounded-2xl border border-indigo-200 bg-white p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-black text-slate-950">Ficha oficial pronta para impressão</h3><p className="mt-1 text-sm text-slate-600">A ficha SINAN completa será gerada; campos não preenchidos permanecem em branco.</p></div><div className="flex flex-col gap-2 sm:flex-row"><button type="button" disabled={pdfAction !== null} onClick={() => handlePdf('save')} className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-700 px-4 py-3 text-sm font-extrabold text-white disabled:bg-slate-300">{pdfAction === 'save' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />} Salvar PDF</button><button type="button" disabled={pdfAction !== null} onClick={() => handlePdf('print')} className="inline-flex items-center justify-center gap-2 rounded-xl border border-indigo-300 bg-indigo-50 px-4 py-3 text-sm font-extrabold text-indigo-950 disabled:text-slate-400">{pdfAction === 'print' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Printer className="h-4 w-4" />} Imprimir ficha</button></div></div>
        {pdfError && <p role="alert" className="mt-3 rounded-xl border border-red-200 bg-red-50 p-3 text-sm font-bold text-red-800">{pdfError}</p>}
      </div>
    </div>
  </section>
}
