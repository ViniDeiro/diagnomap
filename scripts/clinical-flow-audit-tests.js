...
assert.equal(ituFlowchart.steps.itu_bacteriuria_especialista.options.find(option => option.value === 'imunologico_sintomas_localizados')?.nextStep, 'itu_cistite_complicadores', 'ITU: sintomas urinários localizados devem retornar à classificação sintomática')
assert.equal(ituFlowchart.steps.itu_bacteriuria_especialista.options.find(option => option.value === 'imunologico_sistemico_instavel')?.nextStep, 'itu_estabilizacao_sepse', 'ITU: sinais sistêmicos/instabilidade não podem oferecer novamente o caminho de paciente estável')
...
assert.match(universalAssessmentSource, /Sinais vitais e exame físico/, 'Avaliação universal deve apresentar sinais vitais e exame físico na mesma tela')
assert.match(universalAssessmentSource, /showTemperature=\{false\}/, 'Avaliação universal não deve repetir temperatura dentro do exame físico')
assert.doesNotMatch(universalAssessmentSource, /setStage\(|Continuar para exame físico|Revisar sinais vitais/, 'Avaliação universal não deve separar sinais vitais e exame físico em telas diferentes')
assert.match(physicalExamSource, /showTemperature\?: boolean[\s\S]*showTemperature &&/, 'Formulário de exame físico deve permitir ocultar temperatura quando os sinais vitais estiverem na mesma tela')
...
assert.match(flowSource, /value: 'saba_ipratropio_mdi'/, 'Asma deve permitir selecionar bombinha com espaçador')
assert.match(flowSource, /Salbutamol spray 100 mcg\/dose: 4–8 jatos com espaçador \+ ipratrópio spray 20 mcg\/dose: 4–8 jatos[\s\S]*cada 20 minutos durante a primeira hora/, 'Bombinha da asma deve informar apresentação, quantidade de jatos, associação com ipratrópio e frequência')
assert.doesNotMatch(flowSource, /Salbutamol:<\/strong> 4–10 jatos|Salbutamol 4–10 jatos/, 'Asma não deve manter orientação conflitante de 4–10 jatos de salbutamol')
assert.match(flowSource, /value: 'saba_ipratropio_nebulizacao'/, 'Asma deve permitir selecionar nebulização')
assert.match(flowSource, /fenoterol 5 mg\/mL, 10–20 gotas[\s\S]*salbutamol 5 mg\/mL, 10–20 gotas[\s\S]*ipratrópio 0,25 mg\/mL, 20–40 gotas[\s\S]*volume de 5 mL[\s\S]*cada 20 minutos na primeira hora/, 'Nebulização da asma deve informar preparo e repetição completos, com salbutamol e ipratrópio em gotas')
assert.match(flowSource, /text: 'Bombinha com espaçador'[\s\S]*nextStep: 'asma_corticoide_grave_vida'/, 'Escolha por bombinha deve prosseguir ao corticoide')
assert.match(flowSource, /text: 'Nebulização'[\s\S]*nextStep: 'asma_corticoide_grave_vida'/, 'Escolha por nebulização deve prosseguir ao corticoide')
...
assert.match(emergencyComponentSource, /ASTHMA_NEBULIZATION_PRESCRIPTION/, 'Asma deve ter o texto de prescrição sugerida da nebulização combinada')
assert.match(emergencyComponentSource, /data-asthma-copy-nebulization/, 'Tela de broncodilatação da asma deve ter botão de copiar prescrição sugerida após selecionar nebulização')
assert.match(emergencyComponentSource, /isAsthmaBronchodilationChoiceStep[\s\S]*aria-pressed=\{selected\}[\s\S]*Confirmar forma de administração/, 'Asma deve exigir marcação explícita de bombinha ou nebulização antes de avançar')
assert.match(emergencyComponentSource, /disabled=\{!selectedAsthmaBronchodilationRoute\}/, 'Confirmação da via de broncodilatação deve ficar bloqueada sem escolha')
assert.match(clinicalSummarySource, /answers\.asma_nebulizacao_grave_vida === 'saba_ipratropio_mdi'[\s\S]*bombinha com espaçador[\s\S]*saba_ipratropio_nebulizacao'[\s\S]*nebulização/, 'Resumo da asma deve registrar a via de broncodilatação marcada')
...
assert.match(emergencyComponentSource, /flowchart\.id !== 'asthma'[\s\S]*currentStepData\.generatesPrescription === true[\s\S]*currentStepData\.type === 'medication'[\s\S]*currentStepData\.prescriptionTemplate\?\.length/, 'Cópia genérica deve aparecer somente em prescrição ou medicação explicitamente estruturada')
assert.doesNotMatch(emergencyComponentSource, /\/conduta\|tratamento\|terapia\|antibi/, 'Palavras soltas no conteúdo não devem ativar o botão de cópia')
assert.match(flowSource, /asma_tratamento_1h_grave_vida:[\s\S]*Prioridade máxima[\s\S]*A primeira hora começa agora/, 'Tratamento grave da asma deve manter o novo desenho sem ação genérica de cópia')
...
assert.match(emergencyComponentSource, /isInlineCopyableConduct[\s\S]*currentStepData\.generatesPrescription[\s\S]*currentStepData\.type === 'medication'/, 'Somente etapas estruturadas de medicação ou prescrição devem ativar cópia contextual')
assert.match(emergencyComponentSource, /data-asthma-copy-magnesium/, 'Asma: preparo do magnésio deve manter cópia específica')
assert.match(emergencyComponentSource, /data-asthma-copy-discharge/, 'Asma: alta deve manter somente a cópia específica da prescrição')
